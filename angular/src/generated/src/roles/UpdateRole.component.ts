...
export class UpdateRoleComponent extends BaseComponent {
  ...
  ngOnInit(): void {
    ...
    this.http.get("http://localhost:3004/roles/:roleId".replace(":roleId", this.id))
    ...
    this.http.patch("http://localhost:3004/roles/:roleId".replace(":roleId", this.id), this.entity)
    .subscribe((response: RoleType) => {
