...
export function importList(list: IUser[], next) {
  console.log("importList", list);

  async.each(
    list,
    function(data, callback) {
      User.create(data, function(err, savedRow) {
        if (err) {
          console.log(err);
          return callback(err);
        }
...
export function xmlUser(req: Request, res: express.Response, next: express.NextFunction) {
