...
export function importList(list: IRole[], next) {
  console.log("importList", list);

  async.each(
    list,
    function(data, callback) {
      Role.create(data, function(err, savedRow) {
        if (err) {
          console.log(err);
          return callback(err);
        }
...
export function xmlRole(req: Request, res: express.Response, next: express.NextFunction) {
