...
*/
  ...
  where = _.map(where, (operator: WhereQuery, path: string) => {
    ...
    switch (operator.operator) {
      case Operators.IN:
        const options = RoleSchema.path(path);
  ...
  sort = _.map(sort, (s: Order, key: string) => {
    const options = RoleSchema.path(key);
    if (!options) {
...
export function listRole(req: Request, res: express.Response, next: express.NextFunction) {
  ...
  console.log("listRole:", JSON.stringify(query));
  ...
  try {
    ...
    querys[0].exec(function(err, mlist: IRoleModel[]) {
      ...
      return querys[1].exec(function(err2, count: number) {
        ...
        req.roles = new Pagination<IRoleModel>(mlist, count, query.offset, query.limit);
