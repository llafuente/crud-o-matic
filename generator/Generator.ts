...
export class Generator {
  ...
  generateServerAt(schema: Schema, path: string) {
    ...
    // src
    this.template(schema, join(__dirname, "../templates/express/src/app.ts"), join(path, "src", "app.ts"));

    this.template(schema, join(__dirname, "../templates/express/src/auth.ts"), join(path, "src", "auth.ts"));

    this.copy(join(__dirname, "../templates/common.ts"), join(path, "src", "common.ts"));

    this.copy(join(__dirname, "../templates/express/src/HttpError.ts"), join(path, "src", "HttpError.ts"));
